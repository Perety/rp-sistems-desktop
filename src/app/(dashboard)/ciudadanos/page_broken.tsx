        ...
        <div className="max-w-6xl mx-auto">
            ...
            <Modal isOpen={modal} onClose={() => setModal(false)} title="Registrar Ciudadano">
                <div className="space-y-4">
                    ...
                    <tbody>
                        {filtered.map((c, i) => (
                            <motion.tr key={c.id} className="border-t" style={{ borderColor: 'var(--border)', background: i % 2 === 0 ? 'var(--bg-card)' : 'var(--bg-secondary)' }}
                                initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}>
                                <td className="px-4 py-3 font-bold">{c.nombre}</td>
                                <td className="px-4 py-3" style={{ color: 'var(--text-secondary)' }}>{c.dni || '—'}</td>
                                <td className="px-4 py-3" style={{ color: 'var(--text-secondary)' }}>{c.telefono || '—'}</td>
                                <td className="px-4 py-3" style={{ color: 'var(--text-muted)' }}>{c.oficial_registro || '—'}</td>
                                <td className="px-4 py-3" style={{ color: 'var(--text-muted)' }}>{formatDate(c.created_at)}</td>
                            </motion.tr>
                        ))}
                        {filtered.length === 0 && (
                            <tr><td colSpan={5} className="px-4 py-8 text-center text-[13px]" style={{ color: 'var(--text-muted)' }}>Sin registros</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
            <Modal isOpen={modal} onClose={() => setModal(false)} title="Registrar Ciudadano">
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div><label>DNI</label><input type="text" value={form.dni} onChange={(e) => setForm((p) => ({ ...p, dni: e.target.value }))} /></div>
                        <div><label>Teléfono</label><input type="text" value={form.telefono} onChange={(e) => setForm((p) => ({ ...p, telefono: e.target.value }))} /></div>
                    </div>
                    <div><label>Dirección</label><input type="text" value={form.direccion} onChange={(e) => setForm((p) => ({ ...p, direccion: e.target.value }))} /></div>
                    <div><label>Notas</label><textarea value={form.notas} onChange={(e) => setForm((p) => ({ ...p, notas: e.target.value }))} /></div>
